...
#     # Phân tích và xử lý yêu cầu từ người dùng
#     if "đặt lịch hẹn" in user_input:
#         # Giả sử bạn đã có logic để trích xuất các thông tin cần thiết từ user_input
#         ma_benh_nhan = 1  # Mã bệnh nhân, bạn cần thay thế bằng dữ liệu thực tế
#         ngay_hen = "2024-07-01"
#         gio_hen = "09:00:00"
#         ly_do = "Khám sức khỏe định kỳ"
#         result = add_appointment(ma_benh_nhan, ngay_hen, gio_hen, ly_do)
#         return result
#     # Tương tự cho các yêu cầu khác

# # Tạo hàm để gửi yêu cầu thêm lịch hẹn
# def add_appointment(ma_benh_nhan, ngay_hen, gio_hen, ly_do, trang_thai='pending'):
#     url = "http://localhost:5000/add_appointment"
#     payload = {
...
# # Ví dụ gọi hàm với đầu vào từ người dùng
# # user_input = "Tôi muốn đặt lịch hẹn khám vào ngày 1 tháng 7 lúc 9 giờ sáng"
# # response = process_user_input(user_input)
# # print(response)


# # Gọi API chức năng đặt lịch hẹn (Function to call the add_appointment API)
# def call_add_appointment_api(ma_benh_nhan, ngay_hen, gio_hen, ly_do, trang_thai='pending'):
#     url = "http://localhost:5000/add_appointment"
#     payload = {
#         "ma_benh_nhan": ma_benh_nhan,
#         "ngay_hen": ngay_hen,
...
# # API add dữ liệu lịch khám
# @app.route('/add_appointment', methods=['POST'])
# def add_appointment():
#     data = request.get_json()
#     ma_benh_nhan = data.get('ma_benh_nhan')
#     ngay_hen = data.get('ngay_hen')
#     gio_hen = data.get('gio_hen')
#     ly_do = data.get('ly_do')
#     trang_thai = data.get('trang_thai', 'pending')  # Default status is pending

#     query = "INSERT INTO lich_hen (ma_benh_nhan, ngay_hen, gio_hen, ly_do, trang_thai) VALUES (%s, %s, %s, %s, %s)"
#     params = (ma_benh_nhan, ngay_hen, gio_hen, ly_do, trang_thai)
#     db.run(query, params)

#     return jsonify({"message": "Lịch hẹn đã được thêm thành công"}), 201
# # endregion

# # Định hình main cho file
# if __name__ == '__main__':
#     app.run(port=5000, debug=True)
